...
public struct IntRectangle : IEquatable<Rectangle>
{
    ...
    public IntRectangle AddPosition(Vector2 position) => new IntRectangle(X + Maths.RoundToInt(position.X), Y + Maths.RoundToInt(position.Y), Width, Height);
    ...
    {
        X = Maths.RoundToInt(x);
        Y = Maths.RoundToInt(y);
        Width = Maths.RoundToInt(width);
        Height = Maths.RoundToInt(height);
    }
    ...
    {
        int clampedX = System.Math.Max(X, System.Math.Min(innerRect.X, X + Width - innerRect.Width));
        int clampedY = System.Math.Max(Y, System.Math.Min(innerRect.Y, Y + Height - innerRect.Height));
